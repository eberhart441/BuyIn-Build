...
class AQP:
    ...
    def retreveQueryData(self, ALGOquery):
        ...
        closeZScores = (4 * (tickerDataClose - np.mean(tickerDataClose))) / np.std(
            tickerDataClose
        )
        volumeZScores = (tickerDataVolume - np.mean(tickerDataVolume)) / np.std(
            tickerDataVolume
        )

        # Concatenate the z-scores
        tickerDataZScores = np.column_stack(
            [tickerDataTimeframe, closeZScores, volumeZScores]
        )
